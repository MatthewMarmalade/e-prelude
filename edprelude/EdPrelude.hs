...
module EdPrelude (
    ...
    sort, sortOn, sortBy, nub, nubBy, unlines, lines, unwords, words, concatMap, transpose, (\\), genericLength,
...
import Data.List (sort,sortOn,sortBy,nub,nubBy,genericLength,(\\),transpose,intersperse,head,tail,init,last,
                  repeat,map,filter,reverse,delete,takeWhile,dropWhile,zip,unzip,zipWith,isPrefixOf)
import Data.String (lines,unlines,words,unwords)
import Data.Char (isDigit,isAlpha,isAlphaNum,toLower,toUpper,isLower,isUpper)
import Data.Tuple (fst,snd,curry,uncurry)
import GHC.Show
import GHC.Types (Char, Bool(True,False), Double)
import GHC.Base (String, error, errorWithoutStackTrace, ($), (.), undefined, seq, sequence, id, (||), (&&), otherwise, not)
import Data.Maybe
